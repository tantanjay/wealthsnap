...
- [x] **Data Management (Backup, Restore, & Clear Data):** implemented — `tsc`/`eslint` clean, not yet device-tested.
  - **Clear Data:** `storageService.ts`'s `clearAllData()` now drops `savings_goals` too (no separate transactions ledger table exists per the single-table schema decision — goal-tagged rows in `transactions` are already cleared by the existing statement).
  - **Backup & Restore:** `savingsGoals` registered in `backupEntities.ts`'s `ENTITY_REGISTRY` (mirrors the `debts` entry) with a `savingsGoalId` FK on the `transactions` entry; `backupService.ts`'s one hard-coded nested remap (`remapRecurrenceTemplateFks`, for a `RecurrenceRule.transactionTemplate`'s embedded FKs) now also remaps `savingsGoalId` so a restored goal's auto-contribution rule doesn't keep firing against a stale pre-restore id. `exportService.ts` gained two new XLSX sheets ("Savings Goals" and "Goal Transactions"), mirroring the existing Debts/Debt Payments pair. `importService.ts` confirmed to need no changes (plain CSV bank-statement importer, no entity-linking of any kind).
  - **Live Sync:** `savingsGoals` registered in `syncEntities.ts`'s `SYNC_ENTITY_REGISTRY`, reusing `bulkSaveSavingsGoals` as the merge-safe upsert (same shortcut `debts` already uses, since it preserves incoming timestamps). **Found and fixed a real bug while wiring this up:** `deleteSavingsGoal`'s tombstone was recorded under `'savings_goals'` (the SQL table name) instead of `'savingsGoals'` (the registry key) — every other entity in the codebase tombstones under its registry key (e.g. `recurrenceRules`, not `recurrence_rules`), and the mismatch would have silently stopped a deleted goal from ever propagating to another device during merge sync.

...
## 📦 Backup & Restore
Behind-the-scenes work to make backups more reliable, plus a new hands-off option.

- **Auto Backup**: turn on automatic encrypted backups from Profile → Auto Backup instead of remembering to back one up yourself. Pick a weekly or every-2-weeks frequency, set a dedicated backup password once (stored securely on this device so backups can run in the background without asking you each time), and — on Android — choose a destination folder, including a synced cloud-drive folder. The folder picker checks the folder actually supports automatic writes before accepting it, since some cloud providers (Google Drive included) grant folder access but then silently reject file creation — you'll get a clear message on the spot instead of a backup quietly failing weeks later. On iOS, auto-backups are saved to WealthSnap's own on-device folder; open the Files app (On My iPhone → WealthSnap) to move a copy to iCloud Drive or elsewhere. Nothing in your chosen folder is ever deleted or overwritten automatically — cleanup is entirely up to you. Auto-backups are also marked as such (in the filename and inside the backup file itself) so they're never confused with a manual one.

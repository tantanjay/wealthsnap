...
- **Stronger, faster local encryption**: your on-device encryption key was previously passed through an extra derivation step on every single encrypt/decrypt, which both weakened how much of the key's actual strength was used and cost time on every operation. Encryption now uses your device key directly, closing that gap and making local encrypt/decrypt noticeably faster — measured at over 2x faster per operation in testing. Existing data isn't affected and keeps working exactly as before; only newly-written data uses the faster path.

...
const InsightsScreen = ({ navigation }: any) => {
    ...
    const [processedData, setProcessedData] = useState<Metrics.ProcessedData | null>(null);
    ...
    const fetchTransactions = async () => {
        ...
        try {
            ...
            // Consolidate all independent data fetching early
            ...
            await calculateMetrics(allTransactions, allBudgets);
    ...
    const calculateMetrics = useCallback(async (currentTransactions: Transaction[] = transactions, budgetsData?: any[]) => {
        if (!currentTransactions || currentTransactions.length === 0) return;

        // 1. PROCESS DATA ONCE ($O(N)$ once)
        const processed = Metrics.processTransactions(currentTransactions);
        ...
        // 2. GET SUBSETS (Zero cost lookup)
        const currentMonthTrans = Metrics.getTransactionsByMonthKey(processed, monthKey);
        const lastMonthTrans = Metrics.getTransactionsByMonthKey(processed, lastMonthKey);
        ...
        const burnRate = Metrics.calculateBurnRate(processed);
        ...
        // Trends
        const monthlyTrends = Metrics.getMonthlyTrends(processed, 6);

        // Comparison
        const lastMonthTotals = Metrics.calculateTotals(lastMonthTrans);
        const average6Month = Metrics.calculateBurnRate(processed, 6);
        const average1Year = Metrics.calculateBurnRate(processed, 12);
        ...
        // Use provided budgets or fetch if missing (fallback)
        const budgets = budgetsData || await getBudgets();
        ...
        setData({
            ...
            averageExpense: Metrics.calculateBurnRate(processed, 3), // 3-month average
            average6Month,
            average1Year,
            anomalies,
            // New metrics
